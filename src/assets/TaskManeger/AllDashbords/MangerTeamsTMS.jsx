...
//snehal added 27-01-2026 start limit member name'
  ...
  const fetchDashboardStats = async () => {
    try {
      ...
      const [teamsRes, managersRes, employeesRes, departmentsRes] =
        await Promise.all([
          axios.get(`https://cws-ems-server.vercel.app/api/teams/createdBy/${user._id}`),
          axios.get("https://cws-ems-server.vercel.app/managers", {
            headers: { Authorization: `Bearer ${token}` },
          }),
          axios.get("https://cws-ems-server.vercel.app/getEmployeeCount", {}),
  ...
  const fetchTeams = async () => {
    try {
      ...
      const res = await axios.get(
        `https://cws-ems-server.vercel.app/api/teams/createdBy/${user._id}`,
      );
  ...
  const fetchTeamLeaders = async () => {
    try {
      const token = localStorage.getItem("accessToken");
      const response = await axios.get("https://cws-ems-server.vercel.app/teamLead", {
        headers: { Authorization: `Bearer ${token}` },
      });
    ...
    const fetchAddTaskRequiredDetails = async () => {
      try {
        ...
        const empRes = await axios.get(
          `https://cws-ems-server.vercel.app/employees/manager/${user._id}`,
        );
        const projectRes = await axios.get(
          `https://cws-ems-server.vercel.app/api/projects/manager/${user._id}`,
        );
  ...
  async function handleAddTeam(e) {
    try {
      ...
      if (editTaskId) {
        res = await axios.put(
          `https://cws-ems-server.vercel.app/api/teams/${editTaskId}`,
          payload,
        ...
        const res = await axios.post(
          "https://cws-ems-server.vercel.app/api/teams",
          payload,
          { headers: { "Content-Type": "application/json" } },
        );
...
const projectStatus = team?.project?.status?.toLowerCase() || "";
  ...
  const handleDeleteTeam = async (id) => {
    ...
    try {
      await axios.delete(`https://cws-ems-server.vercel.app/api/teams/${id}`);
