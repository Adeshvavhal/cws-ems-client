...
const EmployeeTaskTMS = ({ user }) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`https://cws-ems-server.vercel.app/tasks/assigned/${user._id}`)
      .then((res) => {
...
if (activeTask) {
 ...
 //snehal code timer end
  ...
  useEffect(() => {
    fetch("https://cws-ems-server.vercel.app/unique")
      .then((res) => res.json())
      .then((data) => {
        if (data.success) {
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `https://cws-ems-server.vercel.app/task/${selectedTask._id}/status`,
        {
          method: "PUT",
  ...
  const handleStartTimer = async (taskId) => {
    ...
    try {
      const response = await axios.post(
        `https://cws-ems-server.vercel.app/task/${taskId}/start`,
      );
...
setTimerSeconds(previousSeconds); //  prevents 000 flash
 //snehal code timer end
  ...
  //   try {
  //     const response = await axios.post(
  //       `https://cws-ems-server.vercel.app/task/${taskId}/stop`,
  //     );
  //     if (response.data.success) {
  //       setActiveTimer(null);
  //       alert(
...
//snehal code
  ...
  const handleStartTask = async (task) => {
    ...
    try {
      const statusRes = await fetch(
        `https://cws-ems-server.vercel.app/task/${task._id}/status`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  const handleCompleteTask = async (task) => {
    ...
    try {
      const res = await fetch(`https://cws-ems-server.vercel.app/task/${task._id}/status`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
