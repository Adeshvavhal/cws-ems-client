...
function EmployeeMyLeave({ user, refreshKey,fetchNotifications}) {
  ...
  useEffect(() => {
    ...
    const fetchLeaves = async () => {
      try {
        const res = await axios.get(`https://cws-ems-server.vercel.app/leave/my/${user._id}`);
        ...
        const getName = async (id) => {
          ...
          try {
            const r = await axios.get(`https://cws-ems-server.vercel.app/users/${id}`);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`https://cws-ems-server.vercel.app/leave/${id}`, { method: "DELETE" });
      if (!res.ok) {
