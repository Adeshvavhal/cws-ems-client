...
function TeamLeaderDashboard({ user }) {
  ...
  const fetchData = async () => {
    try {
      const leavesRes = await axios.get(
        `https://cws-ems-server.vercel.app/leave/team-leader/${user._id}`,
      );
      setLeaves(leavesRes.data.leaves || []);
      
      const regRes = await axios.get(
        `https://cws-ems-server.vercel.app/regularization/team-leader/${user._id}`,
      );
  ...
  const updateRegularizationStatus = async (attendanceId, status) => {
    try {
      ...
      await axios.put(
        `https://cws-ems-server.vercel.app/attendance/regularization/${attendanceId}/status`,
        { status },
