...
function EmployeeCheckInOut({ employeeId }) {
  ...
  useEffect(() => {
    ...
    axios
      .get(`https://cws-ems-server.vercel.app/attendance/today/${employeeId}`, {
  ...
  const handleCheckIn = async () => {
    const res = await axios.post(
      `https://cws-ems-server.vercel.app/attendance/checkin/${employeeId}`,
      {},
      { headers: { Authorization: `Bearer ${token}` } },
    );
    setAttendance(res.data);
  ...
  const handleCheckOut = async () => {
    const res = await axios.post(
      `https://cws-ems-server.vercel.app/attendance/checkout/${employeeId}`,
      {},
      { headers: { Authorization: `Bearer ${token}` } },
    );
    setAttendance(res.data);
